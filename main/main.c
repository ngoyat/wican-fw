/*
 * main.c — WiCAN stock main, unmodified except for the #include order fix.
 * GPIO mapping for WICAN_CUSTOM is fully handled by hw_config.h.
 * This file is kept identical to meatpiHQ/wican-fw main.c so all
 * function signatures and call patterns are correct.
 *
 * Copyright (C) 2022  Meatpi Electronics.
 * Written by Ali Slim <ali@meatpi.com>
 * GPL-3.0 license — see project root for full text.
 */
#include <inttypes.h>
...
void app_main(void)
{
    ...
    if (esp_ota_get_partition_description(running, &running_app_info) == ESP_OK)
    {
		...
		sprintf(hardware_version, "%s", HARDWARE_VERSION);
        ...
        {
        	ESP_LOGI(TAG, "project_hardware_rev: OBD (WICAN_CUSTOM mapped as V300)");
